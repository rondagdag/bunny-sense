...
* NOTES:       none
*****************************************************************************/
float* Ringbuf_Pop_Front(
	RING_BUFFER* b)
{
	float* data = NULL;  /* return value */
...
{       /* one element to add to the ring */
	...
	float* ring_data = NULL;     /* used to help point ring data */
...
void Ringbuf_Init(
	RING_BUFFER* b,    /* ring buffer structure */
	float* data, /* data block or array of data */
...
void testRingBufSize16(
	Test* pTest)
{
	float data_element[5];
	float data_store[sizeof(data_element) * 16];
...
void testRingBufSize32(
	Test* pTest)
{
	float data_element[16];
	float data_store[sizeof(data_element) * 32];
